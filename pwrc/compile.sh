...
echo "compiling pwrc core..."
find src/java/pwrc/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp || exit 1
echo "pwrc core class files compiled successfully"

echo "compiling pwrc desktop..."
find src/java/nxtdesktop/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp
if [ $? -eq 0 ]; then
    echo "pwrc desktop class files compiled successfully"
else
    echo "if javafx is not supported, pwrc desktop compile errors are safe to ignore, but desktop wallet will not be available"
fi
