...
jar/exe/dmg package, or run "java -jar pwrc-client.jar" to start the
installer. See https://bitbucket.org/JeanLucPicard/pwrc/issues/283 for more
details about the IzPack installer. After installation, use the shortcuts or
desktop icons to start the Pwrc server.


Using the pwrc-client.zip package:

Unpack the pwrc-client.zip package and open a shell in the resulting pwrc
...
directory, it starts the java process in the background and uses ~/.pwrc for
...
message "Pwrc server 1.x.x started successfully" in the console log. If run in
...
where the Pwrc UI should now be available.
...
to the Pwrc installation directory, as the use of special characters may result
...
conf/pwrc-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
pwrc-default.properties directly, but create a new conf/pwrc.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from pwrc-default.properties, the
settings in pwrc.properties override those in pwrc-default.properties. This way,
when upgrading the software, you can safely overwrite pwrc-default.properties
with the updated file from the new package, while your customizations remain
safe in the pwrc.properties file.


How to contribute?

There are many ways to contribute to Pwrc. Here are some examples:
...
The Pwrc software is a client-server application. It consists of a java server
...
requests which other Pwrc client applications could use.
...
The default Pwrc client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file pwrc.log, which gets
overwritten at restart. In case of an error, the pwrc.log file may contain
...
pwrc.jar file using the enclosed jar.sh script, and then pwrc.jar should be
