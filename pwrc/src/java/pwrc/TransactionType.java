...
import pwrc.Account.ControlType;
import pwrc.AccountLedger.LedgerEvent;
import pwrc.Attachment.AbstractAttachment;
import pwrc.NxtException.ValidationException;
import pwrc.VoteWeighting.VotingModel;
    ...
    public static abstract class Messaging extends TransactionType {
        ...
        public final static TransactionType ARBITRARY_MESSAGE = new Messaging() {
            ...
            void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
                ...
                if (transaction.getRecipientId() == Genesis.CREATOR_ID && Pwrc.getBlockchain().getHeight() > Constants.MONETARY_SYSTEM_BLOCK) {
        ...
        public static final TransactionType ALIAS_ASSIGNMENT = new Messaging() {
            ...
            @Override
            boolean isBlockDuplicate(Transaction transaction, Map<TransactionType, Map<String, Integer>> duplicates) {
                return Pwrc.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK
        ...
        public static final TransactionType HUB_ANNOUNCEMENT = new Messaging() {
            ...
            @Override
            void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
                if (Pwrc.getBlockchain().getHeight() < Constants.TRANSPARENT_FORGING_BLOCK_7) {
                    throw new NxtException.NotYetEnabledException("Hub terminal announcement not yet enabled at height " + Pwrc.getBlockchain().getHeight());
                }
                Attachment.MessagingHubAnnouncement attachment = (Attachment.MessagingHubAnnouncement) transaction.getAttachment();
        ...
        public static final Messaging ACCOUNT_PROPERTY = new Messaging() {
            ...
            void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
                ...
                if (transaction.getAmountNQT() != 0) {
                    throw new NxtException.NotValidException("Account property transaction cannot be used to send PWRC");
                }
                if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
    ...
    public static abstract class ColoredCoins extends TransactionType {
        ...
        public static final TransactionType ASSET_TRANSFER = new ColoredCoins() {
            ...
            void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
                ...
                if (FxtDistribution.ardorSnapshotAssets.contains(attachment.getAssetId()) && Pwrc.getBlockchain().getHeight() >= Constants.IGNIS_BLOCK) {
        ...
        public static final TransactionType DIVIDEND_PAYMENT = new ColoredCoins() {
            ...
            void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
                Attachment.ColoredCoinsDividendPayment attachment = (Attachment.ColoredCoinsDividendPayment)transaction.getAttachment();
                if (attachment.getHeight() > Pwrc.getBlockchain().getHeight()) {
                    throw new NxtException.NotCurrentlyValidException("Invalid dividend payment height: " + attachment.getHeight()
                            + ", must not exceed current blockchain height " + Pwrc.getBlockchain().getHeight());
                }
                ...
                Asset asset;
                if (Pwrc.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK) {
                ...
                if (Pwrc.getBlockchain().getHeight() > Constants.FXT_BLOCK) {
                    AssetDividend lastDividend = AssetDividend.getLastDividend(attachment.getAssetId());
                    if (lastDividend != null && lastDividend.getHeight() > Pwrc.getBlockchain().getHeight() - 60) {
                        throw new NxtException.NotCurrentlyValidException("Last dividend payment for asset " + Long.toUnsignedString(attachment.getAssetId())
                                + " was less than 60 blocks ago at " + lastDividend.getHeight() + ", current height is " + Pwrc.getBlockchain().getHeight()
            ...
            boolean isDuplicate(Transaction transaction, Map<TransactionType, Map<String, Integer>> duplicates) {
                Attachment.ColoredCoinsDividendPayment attachment = (Attachment.ColoredCoinsDividendPayment) transaction.getAttachment();
                return Pwrc.getBlockchain().getHeight() > Constants.FXT_BLOCK &&
    ...
    public static abstract class DigitalGoods extends TransactionType {
        ...
        public static final TransactionType PURCHASE = new DigitalGoods() {
            ...
            void doValidateAttachment(Transaction transaction) throws NxtException.ValidationException {
                ...
                if (attachment.getDeliveryDeadlineTimestamp() <= Pwrc.getBlockchain().getLastBlockTimestamp()) {
            ...
            @Override
            boolean isDuplicate(Transaction transaction, Map<TransactionType, Map<String, Integer>> duplicates) {
                if (Pwrc.getBlockchain().getHeight() < Constants.MONETARY_SYSTEM_BLOCK) {
                    return false;
                }
                Attachment.DigitalGoodsPurchase attachment = (Attachment.DigitalGoodsPurchase) transaction.getAttachment();
                // not a bug, uniqueness is based on DigitalGoods.DELISTING
    ...
    public static abstract class AccountControl extends TransactionType {
        ...
        public static final TransactionType EFFECTIVE_BALANCE_LEASING = new AccountControl() {
            ...
            void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
                ...
                if (recipientPublicKey == null && Pwrc.getBlockchain().getHeight() > Constants.PHASING_BLOCK) {
        ...
        public static final TransactionType SET_PHASING_ONLY = new AccountControl() {
            ...
            void validateAttachment(Transaction transaction) throws ValidationException {
                ...
                if (maxFees < 0 || (maxFees > 0 && maxFees < maxFeesLimit) || maxFees > Constants.MAX_BALANCE_NQT) {
                    throw new NxtException.NotValidException(String.format("Invalid max fees %f PWRC", ((double)maxFees)/Constants.ONE_NXT));
                }
    ...
    public static abstract class Data extends TransactionType {
        ...
        public static final TransactionType TAGGED_DATA_UPLOAD = new Data() {
            ...
            void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
                ...
                if (attachment.getData() == null && Pwrc.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
        ...
        public static final TransactionType TAGGED_DATA_EXTEND = new Data() {
            ...
            void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
                ...
                if ((attachment.jsonIsPruned() || attachment.getData() == null) && Pwrc.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                    throw new NxtException.NotCurrentlyValidException("Data has been pruned prematurely");
                }
                TransactionImpl uploadTransaction = TransactionDb.findTransaction(attachment.getTaggedDataId(), Pwrc.getBlockchain().getHeight());
                ...
                if (taggedData != null && taggedData.getTransactionTimestamp() > Pwrc.getEpochTime() + 6 * Constants.MIN_PRUNABLE_LIFETIME) {
