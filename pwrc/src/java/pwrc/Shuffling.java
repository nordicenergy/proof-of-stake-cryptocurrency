...
import pwrc.crypto.AnonymouslyEncryptedData;
...
public final class Shuffling {
    ...
    private static final boolean deleteFinished = Pwrc.getBooleanProperty("pwrc.deleteFinishedShufflings");
    ...
    private void distribute() {
        ...
        try (DbIterator<ShufflingParticipant> participants = ShufflingParticipant.getParticipants(id)) {
            for (ShufflingParticipant participant : participants) {
                ...
                if (holdingType != HoldingType.PWRC) {
    ...
    private void cancel(Block block) {
        ...
        if (blamedAccountId != 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                Logger.logDebugMessage("Shuffling penalty %f PWRC awarded to forger at height %d", ((double)fee) / Constants.ONE_NXT, block.getHeight() - i - 1);
            }
            ...
            Logger.logDebugMessage("Shuffling penalty %f PWRC awarded to forger at height %d", ((double)fee) / Constants.ONE_NXT, block.getHeight());
        }
