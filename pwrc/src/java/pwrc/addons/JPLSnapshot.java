...
import pwrc.http.APITag;
import pwrc.http.JSONResponses;
import pwrc.http.ParameterParser;
...
public final class JPLSnapshot implements AddOn {
     ...
     * requirement to existing PWRC holders.</p>
     *
     * <p>This utility takes a snapshot of account balances and public keys on the Pwrc blockchain as of the specified height,
     ...
     * <li>height - the Pwrc blockchain height at which to take the snapshot</li>
     * </ul>
     *
     * <p>Response</p>
     ...
     * for a total of 900M to new accounts, resulting in 100M automatically allocated to existing PWRC holders:
     * <pre>
     * {
     *     "balances": {
     *         "PWRC-NZKH-MZRE-2CTT-98NPZ": 30000000000000000,
     *         "PWRC-X5JH-TJKJ-DVGC-5T2V8": 30000000000000000,
     *         "PWRC-LTR8-GMHB-YG56-4NWSE": 30000000000000000
     *     },
     *     "publicKeys": [
    ...
    public static class JPLSnapshotAPI extends APIServlet.APIRequestHandler {
        ...
        protected JSONStreamAware processRequest(HttpServletRequest request, HttpServletResponse response) throws NxtException {
            ...
            if (height <= 0 || height > Pwrc.getBlockchain().getHeight()) {
            ...
            Pwrc.getBlockchainProcessor().addListener(listener, BlockchainProcessor.Event.AFTER_BLOCK_ACCEPT);
            Pwrc.getBlockchainProcessor().scan(height - 1, false);
            Pwrc.getBlockchainProcessor().removeListener(listener, BlockchainProcessor.Event.AFTER_BLOCK_ACCEPT);
