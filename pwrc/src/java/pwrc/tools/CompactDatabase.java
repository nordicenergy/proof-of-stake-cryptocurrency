 ...
 *
 *   java -cp "classes:lib/*:conf" pwrc.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dpwrc.runtime.mode=desktop pwrc.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Pwrc properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "pwrc.testDb" : "pwrc.db";
        String dbType = Pwrc.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Pwrc.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Pwrc.getDbDir(Pwrc.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Pwrc.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Pwrc.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Pwrc.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "pwrc.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "pwrc.mv.db");
            if (!dbFile.exists()) {
        ...
        try {
            ...
            switch (phase) {
                ...
                case 1:
                    ...
                    //
                    File newFile = new File(dbDir, "pwrc.h2.db");
                    if (newFile.exists()) {
                        if (!newFile.delete()) {
                            Logger.logErrorMessage(String.format("Unable to delete '%s'", newFile.getPath()));
                        }
                    } else {
                        newFile = new File(dbDir, "pwrc.mv.db");
                        if (newFile.exists()) {
                            if (!newFile.delete()) {
                                Logger.logErrorMessage(String.format("Unable to delete '%s'", newFile.getPath()));
                            }
