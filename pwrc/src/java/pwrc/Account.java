...
import pwrc.AccountLedger.LedgerEntry;
import pwrc.AccountLedger.LedgerEvent;
import pwrc.AccountLedger.LedgerHolding;
import pwrc.crypto.Crypto;
import pwrc.crypto.EncryptedData;
import pwrc.db.DbClause;
import pwrc.db.DbIterator;
import pwrc.db.DbKey;
import pwrc.db.DbUtils;
import pwrc.db.DerivedDbTable;
import pwrc.db.VersionedEntityDbTable;
import pwrc.db.VersionedPersistentDbTable;
...
public final class Account {
    ...
    public static final class AccountAsset {
        ...
        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO account_asset "
                ...
                pstmt.setInt(++i, Pwrc.getBlockchain().getHeight());
                pstmt.executeUpdate();
            }
        }
    ...
    public static final class PublicKey {
        ...
        private PublicKey(long accountId, byte[] publicKey) {
            ...
            this.height = Pwrc.getBlockchain().getHeight();
        }
        ...
        private void save(Connection con) throws SQLException {
            height = Pwrc.getBlockchain().getHeight();
    ...
    private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>("account_asset", accountAssetDbKeyFactory) {
        ...
        public void checkAvailable(int height) {
            if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < Pwrc.getBlockchainProcessor().getMinRollbackHeight()) {
                throw new IllegalArgumentException("Historical data as of height " + height +" not available.");
            }
            if (height > Pwrc.getBlockchain().getHeight()) {
                throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + Pwrc.getBlockchain().getHeight());
            }
        }
    ...
    private static final ConcurrentMap<DbKey, byte[]> publicKeyCache = Pwrc.getBooleanProperty("pwrc.enablePublicKeyCache") ?
    ...
    static {
        ...
        Pwrc.getBlockchainProcessor().addListener(block -> {
        ...
        if (publicKeyCache != null) {
            ...
            Pwrc.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public long getEffectiveBalanceNXT() {
        return getEffectiveBalanceNXT(Pwrc.getBlockchain().getHeight());
    }
    ...
    public long getEffectiveBalanceNXT(int height) {
        ...
        if (height < Constants.TRANSPARENT_FORGING_BLOCK_3) {
            ...
            for (Transaction transaction : Pwrc.getBlockchain().getBlockAtHeight(height).getTransactions()) {
    ...
    private long getLessorsGuaranteedBalanceNQT(int height) {
        ...
        int blockchainHeight = Pwrc.getBlockchain().getHeight();
        try (Connection con = Db.db.getConnection();
    ...
    public long getGuaranteedBalanceNQT() {
        return getGuaranteedBalanceNQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, Pwrc.getBlockchain().getHeight());
    }
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations, final int currentHeight) {
        ...
        try {
            ...
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < Pwrc.getBlockchainProcessor().getMinRollbackHeight()
                    || height > Pwrc.getBlockchain().getHeight()) {
    ...
    void leaseEffectiveBalance(long lesseeId, int period) {
        int height = Pwrc.getBlockchain().getHeight();
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = Pwrc.getBlockchain().getHeight();
            return true;
        }
    ...
    void apply(byte[] key) {
        ...
        } else if (publicKey.height >= Pwrc.getBlockchain().getHeight() - 1) {
