...
import pwrc.AccountRestrictions;
import pwrc.Alias;
import pwrc.Appendix;
import pwrc.Asset;
import pwrc.AssetDelete;
import pwrc.AssetDividend;
import pwrc.AssetTransfer;
import pwrc.Attachment;
import pwrc.Block;
import pwrc.Constants;
import pwrc.Currency;
import pwrc.CurrencyExchangeOffer;
import pwrc.CurrencyFounder;
import pwrc.CurrencyTransfer;
import pwrc.CurrencyType;
import pwrc.DigitalGoodsStore;
import pwrc.Exchange;
import pwrc.ExchangeRequest;
import pwrc.FundingMonitor;
import pwrc.Generator;
import pwrc.HoldingType;
import pwrc.MonetarySystem;
import pwrc.Pwrc;
import pwrc.Order;
import pwrc.PhasingPoll;
import pwrc.PhasingVote;
import pwrc.Poll;
import pwrc.PrunableMessage;
import pwrc.Shuffler;
import pwrc.Shuffling;
import pwrc.ShufflingParticipant;
...
public final class JSONData {
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, Pwrc.getBlockchain().getHeight());
    }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.PWRC) {
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> phasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult phasingPollResult : phasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(Pwrc.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Pwrc.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", Pwrc.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", Pwrc.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = Pwrc.getBlockchain().getTransaction(entry.getEventId());
