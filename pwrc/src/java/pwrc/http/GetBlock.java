...
import static pwrc.http.JSONResponses.INCORRECT_BLOCK;
import static pwrc.http.JSONResponses.INCORRECT_HEIGHT;
import static pwrc.http.JSONResponses.INCORRECT_TIMESTAMP;
import static pwrc.http.JSONResponses.UNKNOWN_BLOCK;
...
public final class GetBlock extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        if (blockValue != null) {
            try {
                blockData = Pwrc.getBlockchain().getBlock(Convert.parseUnsignedLong(blockValue));
            ...
            try {
                ...
                blockData = Pwrc.getBlockchain().getBlockAtHeight(height);
            } catch (RuntimeException e) {
                return INCORRECT_HEIGHT;
            }
            ...
            try {
                ...
                blockData = Pwrc.getBlockchain().getLastBlock(timestamp);
            } catch (RuntimeException e) {
                return INCORRECT_TIMESTAMP;
            }
        } else {
            blockData = Pwrc.getBlockchain().getLastBlock();
        }
