...
import static pwrc.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static pwrc.http.JSONResponses.INCORRECT_DEADLINE;
import static pwrc.http.JSONResponses.INCORRECT_EC_BLOCK;
import static pwrc.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static pwrc.http.JSONResponses.INCORRECT_WHITELIST;
import static pwrc.http.JSONResponses.MISSING_DEADLINE;
import static pwrc.http.JSONResponses.MISSING_SECRET_PHRASE;
import static pwrc.http.JSONResponses.NOT_ENOUGH_FUNDS;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                Pwrc.getBlockchain().getHeight() + 1,
                Pwrc.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
        ...
        if (ecBlockId != 0 && ecBlockId != Pwrc.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = Pwrc.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = Pwrc.newTransactionBuilder(publicKey, amountNQT, feeNQT,
