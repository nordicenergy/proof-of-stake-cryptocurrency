...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Pwrc.getBooleanProperty("pwrc.enableAPIProxy") && ! API.isOpenAPI);
    private static final int blacklistingPeriod = Pwrc.getIntProperty("pwrc.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Pwrc.getStringProperty("pwrc.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Pwrc.getBlockchainProcessor().isDownloading());
    }
    ...
    boolean blacklistHost(String host) {
        ...
        blacklistedPeers.put(host, Pwrc.getEpochTime() + blacklistingPeriod);
