...
public final class GetNextBlockGenerators extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        if (block == null) {
            curBlock = Pwrc.getBlockchain().getLastBlock();
        } else {
            try {
                curBlock = Pwrc.getBlockchain().getBlock(Convert.parseUnsignedLong(block));
                if (curBlock == null) {
        ...
        */
        ...
        Block curBlock = Pwrc.getBlockchain().getLastBlock();
