...
import static pwrc.http.JSONResponses.MISSING_ADMIN_PASSWORD;
import static pwrc.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static pwrc.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
import static pwrc.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
...
public final class API {
    ...
    public static final String adminPassword = Pwrc.getStringProperty("pwrc.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Pwrc.getIntProperty("pwrc.maxAPIRecords");
    static final boolean enableAPIUPnP = Pwrc.getBooleanProperty("pwrc.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Pwrc.getIntProperty("pwrc.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Pwrc.getBooleanProperty("pwrc.apiServerCORS");
    private static final String forwardedForHeader = Pwrc.getStringProperty("pwrc.forwardedForHeader");
    ...
    static {
        List<String> disabled = new ArrayList<>(Pwrc.getStringListProperty("pwrc.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Pwrc.getStringListProperty("pwrc.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Pwrc.getStringListProperty("pwrc.allowedBotHosts");
        ...
        boolean enableAPIServer = Pwrc.getBooleanProperty("pwrc.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Pwrc.getIntProperty("pwrc.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Pwrc.getIntProperty("pwrc.apiServerSSLPort");
            final String host = Pwrc.getStringProperty("pwrc.apiServerHost");
            disableAdminPassword = Pwrc.getBooleanProperty("pwrc.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Pwrc.getBooleanProperty("pwrc.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Pwrc.getUserHomeDir()).resolve(Paths.get(Pwrc.getStringProperty("pwrc.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Pwrc.getStringProperty("pwrc.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Pwrc.getStringProperty("pwrc.keyStoreType"));
                List<String> ciphers = Pwrc.getStringListProperty("pwrc.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Pwrc.getStringProperty("pwrc.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Pwrc.getStringProperty("pwrc.apiWelcomeFile")});
            }

            String javadocResourceBase = Pwrc.getStringProperty("pwrc.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/pwrc");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Pwrc.getIntProperty("pwrc.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/pwrc-proxy");
            ...
            if (!Pwrc.getBooleanProperty("pwrc.enableAPIServerGZIPFilter", isOpenAPI)) {
                gzipHandler.setExcludedPaths("/pwrc", "/pwrc-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(pwrc.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (Pwrc.getBooleanProperty("pwrc.apiFrameOptionsSameOrigin")) {
