...
public final class DebugTrace {
    ...
    static final String QUOTE = Pwrc.getStringProperty("pwrc.debugTraceQuote", "\"");
    static final String SEPARATOR = Pwrc.getStringProperty("pwrc.debugTraceSeparator", "\t");
    static final boolean LOG_UNCONFIRMED = Pwrc.getBooleanProperty("pwrc.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = Pwrc.getStringListProperty("pwrc.debugTraceAccounts");
        String logName = Pwrc.getStringProperty("pwrc.debugTraceLog");
        ...
        Pwrc.getBlockchainProcessor().addListener(block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        Pwrc.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        Pwrc.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        Pwrc.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
    ...
    private void traceShufflingCancel(Shuffling shuffling) {
        ...
        if (blamedAccountId != 0 && include(blamedAccountId)) {
            ...
            long generatorId = Pwrc.getBlockchain().getLastBlock().getGeneratorId();
            if (include(generatorId)) {
                Map<String,String> generatorMap = getValues(generatorId, false);
                generatorMap.put("generation fee", String.valueOf(fee));
                generatorMap.put("event", "shuffling blame");
    ...
    private Map<String,String> lessorGuaranteedBalance(Account account, long lesseeId) {
        ...
        map.put("timestamp", String.valueOf(Pwrc.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(Pwrc.getBlockchain().getHeight()));
