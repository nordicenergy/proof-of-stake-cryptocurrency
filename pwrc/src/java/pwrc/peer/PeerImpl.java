...
import pwrc.util.CountingInputReader;
import pwrc.util.CountingInputStream;
import pwrc.util.CountingOutputWriter;
...
final class PeerImpl implements Peer {
    ...
    void setVersion(String version) {
        ...
        isOldVersion = false;
        if (Pwrc.APPLICATION.equals(application)) {
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < Pwrc.getBlockchain().getHeight() - 60) {
            long accountId = hallmark.getAccountId();
            Account account = Account.getAccount(accountId);
            hallmarkBalance = account == null ? 0 : account.getBalanceNQT();
            hallmarkBalanceHeight = Pwrc.getBlockchain().getHeight();
        }
    ...
    public void blacklist(String cause) {
        blacklistingTime = Pwrc.getEpochTime();
    ...
    public JSONObject send(final JSONStreamAware request, int maxResponseSize) {
        ...
        try {
            ...
            if (useWebSocket && !webSocket.isOpen())
                useWebSocket = webSocket.startClient(URI.create("ws://" + host + ":" + getPort() + "/pwrc"));
            //
            ...
            if (useWebSocket) {
                ...
                //
                URL url = new URL("http://" + host + ":" + getPort() + "/pwrc");
                if (communicationLoggingMask != 0)
    ...
    void connect() {
        lastConnectAttempt = Pwrc.getEpochTime();
        try {
