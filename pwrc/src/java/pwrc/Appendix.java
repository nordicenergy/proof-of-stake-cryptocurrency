...
public interface Appendix {
    ...
    interface Prunable {
        ...
        default boolean shouldLoadPrunable(Transaction transaction, boolean includeExpiredPrunable) {
            return Pwrc.getEpochTime() - transaction.getTimestamp() <
    ...
    class Message extends AbstractAppendix {
        ...
        void validate(Transaction transaction) throws NxtException.ValidationException {
            if (Pwrc.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && message.length > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
    ...
    class PrunablePlainMessage extends Appendix.AbstractAppendix implements Prunable {
        ...
        void validate(Transaction transaction) throws NxtException.ValidationException {
            ...
            if (msg == null && Pwrc.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
        ...
        @Override
        void apply(Transaction transaction, Account senderAccount, Account recipientAccount) {
            if (Pwrc.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME) {
    ...
    abstract class AbstractEncryptedMessage extends AbstractAppendix {
        ...
        @Override
        void validate(Transaction transaction) throws NxtException.ValidationException {
            if (Pwrc.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && getEncryptedDataLength() > Constants.MAX_ENCRYPTED_MESSAGE_LENGTH) {
    ...
    class PrunableEncryptedMessage extends AbstractAppendix implements Prunable {
        ...
        void validate(Transaction transaction) throws NxtException.ValidationException {
            ...
            if (ed == null && Pwrc.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
    ...
    final class Phasing extends AbstractAppendix {
        ...
        void validate(Transaction transaction) throws NxtException.ValidationException {
            params.validate();
            int currentHeight = Pwrc.getBlockchain().getHeight();
        ...
        void countVotes(TransactionImpl transaction) {
            if (Pwrc.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && PhasingPoll.getResult(transaction.getId()) != null) {
                return;
            }
            ...
            if (result >= poll.getQuorum()) {
                if (!transaction.attachmentIsDuplicate(duplicates, false)) {
                    try {
                        release(transaction);
                        poll.finish(result);
                        Logger.logDebugMessage("Early finish of transaction " + transaction.getStringId() + " at height " + Pwrc.getBlockchain().getHeight());
                    } catch (RuntimeException e) {
                        Logger.logErrorMessage("Failed to release phased transaction " + transaction.getJSONObject().toJSONString(), e);
                    }
                } else {
                    Logger.logDebugMessage("At height " + Pwrc.getBlockchain().getHeight() + " phased transaction " + transaction.getStringId()
