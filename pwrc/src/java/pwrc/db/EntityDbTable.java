...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Pwrc.getBlockchainProcessor().isScanning() ?
                    Math.max(Pwrc.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
    ...
    private boolean doesNotExceed(int height) {
        return Pwrc.getBlockchain().getHeight() <= height && ! (isPersistent() && Pwrc.getBlockchainProcessor().isScanning());
    }
